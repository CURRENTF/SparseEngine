...
def test_prepared_provider_projection_pooling_normalization_and_rope(ratio, dim):
    ...
    completed = provider.compute(projected, view, freqs)
    assert completed.shape == (tokens//ratio, dim)
    ...
    for end in range(ratio, tokens + 1, ratio):
        ...
        torch.testing.assert_close(completed[end//ratio-1], pooled.float(), rtol=0.003, atol=0.0005)
