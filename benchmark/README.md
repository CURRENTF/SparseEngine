...
| `long_bench/` | `pred.py`, `eval.py` | Existing LongBench v1 prediction and scoring through the native Sparse-Engine runtime. |
| `long_bench_v2/` | `pred.py`, `upstream/` | Independent LongBench v2 runner with the official repository pinned as a submodule, untruncated token-stratified selection, and native Sparse-Engine inference. |
...
| `claw_eval/` | `run_sparseengine_claw_eval.sh` | Claw-Eval through the shared Sparse-Engine OpenAI-compatible server. |
...
| `sparseengine_regression/` | `run_suite.py` | Fixed LongBench v1/v2, RULER quality, performance, and stress regression harness. |
