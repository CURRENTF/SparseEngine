...
临时前缀评分通过标准缓存管理器的 `reserve_prefill_slots` 作用域申请资源。
调用方先保护所需的完整缓存路径，再提交有序的 `(seq_id, query_tokens)` 开销。
管理器按普通驱逐策略回收缓存，并为能容纳的前部批次实际预留槽位和缓存行。
模型执行器只构造评分输入、累计分数，不解释空闲槽位数或自行实现驱逐策略。
准入同时计算 CPU 前缀回迁需求，共享块只计一次，并为普通前缀挂接流程留出槽位。

普通 prefill 分配消耗已预留的槽位。退出作用域时归还未使用的预留资源和新领取
的缓存行；已使用的 KV 按普通行生命周期释放。Prefill、decode 和前缀回迁共用
物理槽位领取操作。进入同步维护作用域前必须提交已有异步结果。资源申请成功也
不允许驱逐受保护前缀，包括不属于候选区间的缓存尾部。
