...
def resolve_prefill_token_budget(config) -> None:
    ...
    if (
        ...
        and effective_step_budget < int(config.observation_window_size)
    ):
        raise ValueError(
            f"{method} requires the effective prefill step budget "
            f"min(engine_prefill_chunk_size, max_num_batched_tokens) >= "
            f"observation_window_size so the final score window fits in one prefill "
            f"step: engine_prefill_chunk_size={chunk}, "
            f"max_num_batched_tokens={budget}, "
            f"observation_window_size={config.observation_window_size}."
        )
